...
class Hpx(CMakePackage, CudaPackage, ROCmPackage):
    ...
    variant(
        "networking",
        values=any_combination_of("tcp", "mpi", "lci", "all").with_default("tcp"),
    ...
    depends_on("mpi", when="networking=all")
    depends_on("mpi", when="+async_mpi")
    depends_on("lci", when="networking=lci")
    depends_on("lci", when="networking=all")
    ...
    def cmake_args(self):
        ...
        args += [
            ...
            self.define("HPX_WITH_PARCELPORT_TCP", spec.satisfies("networking=tcp") or spec.satisfies("networking=all")),
            self.define("HPX_WITH_PARCELPORT_MPI", spec.satisfies("networking=mpi") or spec.satisfies("networking=all")),
            self.define(
                "HPX_WITH_MAX_CPU_COUNT",
                format_max_cpu_count(spec.variants["max_cpu_count"].value),
            ),
            self.define("HPX_WITH_PARCELPORT_LCI", spec.satisfies("networking=lci") or spec.satisfies("networking=all")),
